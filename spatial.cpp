...
bool spatial_t::LOS(const vec3f_t &a, const vec3f_t &b) const {
  ...
  const float max_dist = sqrtf(rx*rx + ry*ry + rz*rz);
  ...
  while (true) {
    ...
    if (len.x < len.y) {
      ...
      hit = vec3f_t{px.x, px.y, a.z + rz * dist};
      ...
      hit = vec3f_t{py.x, py.y, a.z + rz * dist};
    }

    if (hit.z <= newfloor || hit.z <= oldfloor) {
      return false;
    }

    if (hit.z >= newceil || hit.z >= oldceil) {
      return false;
    }
...
void spatial_t::hitscan(float vx, float vy, float rx, float ry, vec3f_t &hit, thing_t *&thing) const {
